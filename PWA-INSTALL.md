# GLN Karen Typing Center — PWA Install

## Publish
Upload all files in this folder to the root of the existing GitHub Pages repository.

## Add to iPhone Home Screen
1. Open the GLN Karen Typing Center website in Safari.
2. Tap Share.
3. Tap Add to Home Screen.
4. Keep the name GLN Karen Typing (or edit it).
5. Tap Add.

The GLN logo will be used as the Home Screen icon and the site opens in standalone app mode.

## Notes
- The Type, Practice, and Games interface can be cached for offline use after the first successful visit.
- Classroom Race still needs internet because it uses Firebase for live multiplayer.
- If you update cached files later, change the CACHE name in sw.js (for example v1 to v2) so installed copies refresh cleanly.
